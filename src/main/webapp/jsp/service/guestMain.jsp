...
<table border="1" width="1500">
    ...
    <c:forEach var="room" items="${roomList }">
       ...
       -->
            ...
            <td><a href="<%=request.getContextPath()%>/jsp/service/roomDetail?roomId=${room.id}">방 상세보기</a>
        </tr>
    </c:forEach>
</table>
...
<div>
    ...
    <c:if test="${startPage>10 }">
        <a href="<%=request.getContextPath() %>/guestMain?pageNum=${ startPage - 1}">이전</a>
    </c:if>
    <!--  -->
    <c:forEach var="num" begin="${startPage }" end="${endPage }" >
        <c:choose>
            <c:when test="${pageNum == num }"><!-- 현재 페이지 색 바꾸기 -->
                <a href="<%=request.getContextPath() %>/guestMain?pageNum=${num }">
    ...
    <c:if test="${endPage<pageCount }">
        <a href="<%=request.getContextPath() %>/guestMain?pageNum=${endPage + 1}">다음</a>
    </c:if>
    <!-- -->
</div>
